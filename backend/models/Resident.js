...
const AddResidentModal = ({ onClose, onSaved, doFetch, toast }) => {
    ...
    return (
        <div className="modal-overlay">
            <div className="registration-modal" style={{ maxWidth:580, padding:0 }}>
                <MHeader icon={<FaPlus />} title="Add New Resident" onClose={onClose} />
                <div className="modal-body" style={{ maxHeight:'80vh', overflowY:'auto' }}>

                    {/* Personal Info */}
                    <div style={{ fontSize:'.75rem', fontWeight:700, color:'var(--d-muted)', textTransform:'uppercase', letterSpacing:'.06em', marginBottom:10, paddingBottom:6, borderBottom:'1.5px solid var(--d-border)' }}>
                        Personal Information
                    </div>
                    <div className="form-grid-2">
                        <Field label="First Name" required error={errs.firstName}>
                            <input className={`form-input${errs.firstName?' error':''}`} value={f.firstName} onChange={e=>set('firstName',e.target.value)} placeholder="e.g. Maria" />
                        </Field>
                        <Field label="Last Name" required error={errs.lastName}>
                            <input className={`form-input${errs.lastName?' error':''}`} value={f.lastName} onChange={e=>set('lastName',e.target.value)} placeholder="e.g. Santos" />
                        </Field>
                        <Field label="Middle Name">
                            <input className="form-input" value={f.middleName} onChange={e=>set('middleName',e.target.value)} placeholder="Optional" />
                        </Field>
                        <Field label="Age" required error={errs.age}>
                            <input type="number" min="1" max="130" className={`form-input${errs.age?' error':''}`} value={f.age} onChange={e=>set('age',e.target.value)} placeholder="e.g. 75" />
                        </Field>
                        <Field label="Gender">
                        ...
                        <Field label="Admission Date">
                            <input type="date" className="form-input" value={f.admissionDate} onChange={e=>set('admissionDate',e.target.value)} />
                        </Field>
                    </div>

                    {/* Room Assignment */}
                    <div style={{ fontSize:'.75rem', fontWeight:700, color:'var(--d-muted)', textTransform:'uppercase', letterSpacing:'.06em', margin:'14px 0 10px', paddingBottom:6, borderBottom:'1.5px solid var(--d-border)' }}>
                        Room Assignment
                    </div>
                    <div className="form-grid-2">
                        <Field label="Room Number" required error={errs.roomNumber}>
                            <input className={`form-input${errs.roomNumber?' error':''}`} value={f.roomNumber} onChange={e=>set('roomNumber',e.target.value)} placeholder="e.g. 201" />
                        </Field>
                        <Field label="Floor / Ward" required error={errs.floor}>
                            <select className={`form-input${errs.floor?' error':''}`} value={f.floor} onChange={e=>set('floor',e.target.value)}>
                    ...
                    <Field label="Medical Conditions (comma-separated)">
                        <input className="form-input" value={f.conditions} onChange={e=>set('conditions',e.target.value)} placeholder="e.g. Hypertension, Diabetes, Arthritis" />
                    </Field>
                    <Field label="Primary Nurse (leave blank to auto-assign)">
                        <input className="form-input" value={f.primaryNurse} onChange={e=>set('primaryNurse',e.target.value)} placeholder="Auto-assigned to you if blank" />
                    </Field>
                                ...
                                <tbody>
                                    {logs.map(l => (
                                        <tr key={l._id}>
                                            ...
                                            <td><strong>{l.medicationId?.name || '—'}</strong></td>
                                    ...
                                    <div>
                                        {allDone
                                            ...
                                            : <div className="res-action-btns">
                                                <button className="btn-outline-sm res-btn" onClick={()=>setModal({type:'history',data:r})}>View Full Profile</button>
