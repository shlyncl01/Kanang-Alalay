...
async function dispatchToTokens(tokens, title, body, data) {
    ...
    // so future sends don't keep retrying a dead device.
...
async function sendPushToAll(title, body, data = {}) {
    try {
        ...
        await dispatchToTokens(tokens, title, body, data);
    } catch (error) {
        console.error('[Push] Failed to send push notifications:', error.message);
    }
}

// Targets specific users only — e.g. the caregiver a reminder is actually for,
// instead of every device that happens to be registered.
async function sendPushToUsers(userIds, title, body, data = {}) {
    try {
        ...
        const users = await User.find(
            { _id: { $in: ids }, pushToken: { $exists: true, $ne: null } },
            { pushToken: 1 }
        );
        const tokens = [...new Set(users.map((u) => u.pushToken).filter(Boolean))];
        await dispatchToTokens(tokens, title, body, data);
    } catch (error) {
        console.error('[Push] Failed to send targeted push notifications:', error.message);
    }
}

module.exports = { sendPushToAll, sendPushToUsers };
